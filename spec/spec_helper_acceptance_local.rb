...
RSpec.configure do |c|
  ...
  c.before :suite do
    ...
    if os[:family].match?(%r{redhat})
      run_shell('mv /etc/yum.repos.d/redhat.repo /etc/yum.repos.d/internal-mirror.repo', expect_failures: true)
      run_shell("rpm -ivh https://dl.fedoraproject.org/pub/epel/epel-release-latest-#{os[:release]}.noarch.rpm", expect_failures: true)
    ...
    run_shell('apt-get purge -y container-tools') unless os[:family].match?(%r{windows|redhat}) || !ENV['CI']
    ...
    if os[:family].match?(%r{redhat}) && os[:release].to_i == 7
    ...
    retry_on_error_matching(40, 15, %r{.*}) do
    ...
    retry_on_error_matching(10, 5, %r{.*}) do
