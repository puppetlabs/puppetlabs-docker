...
describe 'docker::compose', :type => :class do
  ...
  context 'with defaults for all parameters' do
    ...
    it { should contain_exec('Install Docker Compose 1.19.0').with(
      'path'    => '/usr/bin/',
      'cwd'     => '/tmp',
      'command' => 'curl -s -S -L  https://github.com/docker/compose/releases/download/1.19.0/docker-compose-Linux-x86_64 -o /usr/local/bin/docker-compose-1.19.0',
      'creates' => '/usr/local/bin/docker-compose-1.19.0',
      'require' => 'Package[curl]'
    )}
    it { should contain_file('/usr/local/bin/docker-compose-1.19.0').with(
      'owner'   => 'root',
      'mode'    => '0755',
      'require' => 'Exec[Install Docker Compose 1.19.0]'
    )}
    it { should contain_file('/usr/local/bin/docker-compose').with(
      'ensure'   => 'link',
      'target'   => '/usr/local/bin/docker-compose-1.19.0',
      'require'  => 'File[/usr/local/bin/docker-compose-1.19.0]'
    )}
  end

  context 'with ensure => absent' do
    let (:params) { { :ensure => 'absent' } }
    it { should contain_file('/usr/local/bin/docker-compose-1.19.0').with_ensure('absent') }
