...
Facter.add(:docker_swarm) do
  setcode do
    ...
    swarm_status = Facter::Core::Execution.exec("/usr/bin/docker info -f '{{json .Swarm.ControlAvailable }}'")
    if docker_binary !~ %r{docker}
      ...
      if service_array
        service_array.each do |service_id|
          service_count += 1
          service_inspect_json = Facter::Core::Execution.exec("/usr/bin/docker inspect #{service_id}")
          service_meta_data = JSON.parse(service_inspect_json)
          hostname = Facter::Core::Execution.exec("/usr/bin/docker inspect -f \"{{ .Name }}\" #{service_id}").sub(%r{^/}, '')
          ip = Facter::Core::Execution.exec("/usr/bin/docker inspect -f \"{{ .NetworkSettings.IPAddress }}\" #{service_id}")
          if hostname
