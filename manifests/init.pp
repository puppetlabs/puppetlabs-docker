...
class docker(
  ...
  Boolean $docker_ee                                  = $docker::params::docker_ee,
  String $docker_ee_package_name                      = $docker::params::package_ee_package_name,
  ...
  String $docker_ee_repos                             = $docker::params::package_ee_repos,
  Optional[String] $docker_ee_release                 = $docker::params::package_ee_release,
  Optional[String] $tcp_bind                          = $docker::params::tcp_bind,
  ...
  Boolean $pin_upstream_package_source                = $docker::params::pin_upstream_package_source,
  Integer $apt_source_pin_level                       = $docker::params::apt_source_pin_level,
  ...
  Optional[Array] $dns                                = $docker::params::dns,
  Optional[Array] $dns_search                         = $docker::params::dns_search,
  Optional[String] $socket_group                      = $docker::params::socket_group,
  Array $labels                                       = $docker::params::labels,
  Optional[Array] $extra_parameters                   = undef,
  Optional[Array] $shell_values                       = undef,
  ...
  Optional[Boolean] $package_source                   = $docker::params::package_source,
  ...
  Optional[Hash] $repo_opt                            = $docker::params::repo_opt,
  ...
  Boolean $service_hasstatus                          = $docker::params::service_hasstatus,
  Boolean $service_hasrestart                         = $docker::params::service_hasrestart,
  ...
  validate_legacy(Boolean, 'validate_bool', $manage_kernel)
  validate_legacy(Boolean, 'validate_bool', $manage_service)
  validate_legacy(Tuple, 'validate_array', $docker_users)
  validate_legacy(Tuple, 'validate_array', $daemon_environment_files)
  validate_legacy(Tuple, 'validate_array', $log_opt)
  validate_legacy(Boolean, 'validate_bool', $tls_enable)
  validate_legacy(Boolean, 'validate_bool', $ip_forward)
  validate_legacy(Boolean, 'validate_bool', $iptables)
  validate_legacy(Boolean, 'validate_bool', $ip_masq)
  if $icc != undef {
    validate_legacy(Boolean, 'validate_bool', $icc)
  }
  validate_legacy(String, 'validate_string', $bridge)
  validate_legacy(String, 'validate_string', $fixed_cidr)
  validate_legacy(String, 'validate_string', $default_gateway)
  validate_legacy(String, 'validate_string', $bip)
  ...
  if $log_level {
    validate_legacy(String, 'validate_re', $log_level, '^(debug|info|warn|error|fatal)$', 'log_level must be one of debug, info, warn, error or fatal')
  }

  if $log_driver {
    validate_legacy(String, 'validate_re', $log_driver, '^(none|json-file|syslog|journald|gelf|fluentd|splunk)$',
                'log_driver must be one of none, json-file, syslog, journald, gelf, fluentd or splunk')
  }

  if $selinux_enabled {
    validate_legacy(String, 'validate_re', $selinux_enabled, '^(true|false)$', 'selinux_enabled must be true or false')
  }

  if $storage_driver {
    validate_legacy(String, 'validate_re', $storage_driver, '^(aufs|devicemapper|btrfs|overlay|overlay2|vfs|zfs)$',
                'Valid values for storage_driver are aufs, devicemapper, btrfs, overlay, overlay2, vfs, zfs.' )
  }

  if $dm_fs {
    validate_legacy(String, 'validate_re', $dm_fs, '^(ext4|xfs)$', 'Only ext4 and xfs are supported currently for dm_fs.')
  }
  ...
  if($tls_enable) {
    ...
    validate_legacy(String, 'validate_string', $tls_cacert)
    validate_legacy(String, 'validate_string', $tls_cert)
    validate_legacy(String, 'validate_string', $tls_key)
  }

  if ( $version == undef ) or ( $version !~ /^(17[.]0[0-5][.]\d(~|-)ce|1.\d+)/ ) {
    if ( $docker_ee) {
      validate_legacy(String, 'validate_string', $docker::docker_ee_source_location)
      validate_legacy(String, 'validate_string', $docker::docker_ee_key_source)
